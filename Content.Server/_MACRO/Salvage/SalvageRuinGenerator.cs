...
public sealed class SalvageRuinGeneratorSystem : EntitySystem
{
    ...
    private HashSet<Vector2i> FloodFillMultiStage(Dictionary<Vector2i, int> costMap, Vector2i start, int stagesCount, int budgetPerStage, HashSet<Vector2i> wallPositions, System.Random rand, int spaceCost, int defaultTileCost)
    {
        ...
        {
			// This is going to look very odd compared to most other ss14 code. This section of code is doing the floodfill entirely in memory without spawning anything, because that's much more performant. 
			...
			// In practice this makes it so that anywhere the floodfill visited, if it's next to a wall, brings in that wall
			// This makes it so that it generally brings in room shapes better than if it only brought in visited tiles. Because the floodfill often runs out of points when it finds a wall.
            var adjacentWithWalls = new HashSet<Vector2i>();
